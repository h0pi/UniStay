...
/// </summary>
public sealed class UniStayExceptionHandler(
    ILogger<UniStayExceptionHandler> logger,
...
) : IExceptionHandler
{
    public async ValueTask<bool> TryHandleAsync(HttpContext ctx, Exception ex, CancellationToken ct)
    {
        ...
        {
            UniStayNotFoundException => StatusCodes.Status404NotFound,
            UniStayConflictException or UniStayBusinessRuleException => StatusCodes.Status409Conflict,
    ...
    private static ErrorDto BuildErrorDto(Exception ex, bool isDev, string traceId)
    {
        ...
        switch (ex)
        {
            case UniStayNotFoundException:
            case UniStayConflictException:
            case UniStayBusinessRuleException:
                code = "entity.error";
                message = ex.Message;
                break;
