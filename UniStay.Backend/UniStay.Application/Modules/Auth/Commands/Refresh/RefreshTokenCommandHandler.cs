namespace UniStay.Application.Modules.Auth.Commands.Refresh;
    ...
    : IRequestHandler<RefreshTokenCommand, RefreshTokenCommandDto>
{
    public async Task<RefreshTokenCommandDto> Handle(RefreshTokenCommand request, CancellationToken ct)
    {
        ...
        if (rt is null || rt.ExpiresAtUtc <= nowUtc)
            throw new UniStayConflictException("Refresh token je nevažeći ili je istekao.");
        ...
        {
            throw new UniStayConflictException("Neispravan klijentski otisak.");
        }

        var user = rt.User;
        if (user is null || /*!user.IsEnabled ||*/ user.IsDeleted)
            throw new UniStayConflictException("Korisnički nalog je nevažeći.");
