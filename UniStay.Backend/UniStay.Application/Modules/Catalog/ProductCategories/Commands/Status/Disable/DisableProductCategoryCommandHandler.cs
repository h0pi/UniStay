namespace UniStay.Application.Modules.Catalog.ProductCategories.Commands.Status.Disable;
    ...
    : IRequestHandler<DisableProductCategoryCommand, Unit>
{
    public async Task<Unit> Handle(DisableProductCategoryCommand request, CancellationToken ct)
    {
        ...
        if (cat is null)
        {
            throw new UniStayNotFoundException($"Kategorija (ID={request.Id}) nije pronađena.");
        }
        ...
        {
            throw new UniStayBusinessRuleException("category.disable.blocked.activeProducts",
