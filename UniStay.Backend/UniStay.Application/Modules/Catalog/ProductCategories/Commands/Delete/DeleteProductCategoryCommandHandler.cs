namespace UniStay.Application.Modules.Catalog.ProductCategories.Commands.Delete;
      ...
      : IRequestHandler<DeleteProductCategoryCommand, Unit>
{
    public async Task<Unit> Handle(DeleteProductCategoryCommand request, CancellationToken cancellationToken)
    {
        if (appCurrentUser.UserId is null)
            throw new UniStayBusinessRuleException("123", "Korisnik nije autentifikovan.");

        var category = await context.ProductCategories
            .FirstOrDefaultAsync(x => x.Id == request.Id, cancellationToken);

        if (category is null)
            throw new UniStayNotFoundException("Kategorija nije pronađena.");
