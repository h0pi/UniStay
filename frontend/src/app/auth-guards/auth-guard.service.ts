...
export class AuthGuard implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot): boolean {
    ...
    if (guardData.isAdmin && !this.authService.isStudent()) {
      this.router.navigate(['/unauthorized']);
      return false;
    }

    // Provjera prava pristupa za menadžera
    if (guardData.isManager && !this.authService.isEmployee()) {
      this.router.navigate(['/unauthorized']);
      return false;
    }

    // Provjera prava pristupa za menadžera
    if (guardData.isManager && !this.authService.isAdministrator()) {
      this.router.navigate(['/unauthorized']);
      return false;
    }
